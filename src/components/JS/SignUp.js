...
import "../CSS/Auth.css";
import { auth, db, createUser } from "../../services/firebase";
...
function SignUP() {
  ...
  const firstnameControl = (event) => {
    ...
    if (event.target.value.length < 3) setFn_error(true);
    else {
      setFn_error(false);
  ...
  const lastnameControl = (event) => {
    console.log(event.target.value);
    if (event.target.value.length < 3) setLs_error(true);
    else {
  ...
  const handleSignUp = () => {
    if (
      ...
      console.log("ready to sign up dude");
    ...
    <div className="signup">
      ...
      <form className="login-form">
        ...
        <TextField
          ...
          label="Lastname"
          placeholder="Chiari"
          helperText="Enter Your lasstname"
          onChange={lastnameControl}
        />
        <TextField
          error={email_error}
          required
